...
class veh_DataProcesser:

    def __init__(self, data_dir, observed_frame_num, predicting_frame_num): # ,dup_threshold):
        self.data_dir = data_dir
        self.file_path = os.path.join(self.data_dir, 'roundabout_traj_veh_filtered.csv')
    ...
    def from_csv(self):
        ...
        self.raw_data = (self.raw_data[12, 1:], self.raw_data[1, 1:], self.raw_data[14, 1:], self.raw_data[15,1:])
        x=2*(self.raw_data[3]-min(self.raw_data[3]))/(max(self.raw_data[3])-min(self.raw_data[3]))-1
        y=2*(self.raw_data[2]-min(self.raw_data[2]))/(max(self.raw_data[2])-min(self.raw_data[2]))-1
    ...
    def get_obs_pred(self):
        ...
        self.obs_veh = np.reshape(self.obs, [count, self.obs_length, 4])
        self.pred_veh = np.reshape(self.pred, [count, self.pred_length, 4])

        return self.obs_veh, self.pred_veh
